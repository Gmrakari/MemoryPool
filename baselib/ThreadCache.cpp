...
/*
 * Date:2021-09-05 10:31
 * filename:ThreadCache.cpp
 *
 */
...
void* ThreadCache::FetchFromCentralCache(size_t index, size_t size) { //index表示freelist中的对应链表下标，size表示内存块大小
	Freelist* freelist = &_freelist[index];

	//每一次取批量的数据，因为每次到CentralCache申请内存的时候是需要加锁的
	//所以一次就多申请一些内存块，防止每次到CentralCache去内存块的时候，多加锁造成效率问题
	//不是每次申请10个，而是进行慢慢增长的过程
	
	//申请的内存块对象越小，能够申请的内存块的数量越多
	//申请的内存块对象越大，能够申请的内存块的数量越少
...
void ThreadCache::ListTooLong(FreeList* firstlist, size_t size) {
...
void* ThreadCache::Allocate(size_t size) {
	...
	//
	//	随着取的次数增加而内存对象个数增加，防止一次给其他线程分配太多，而另一些线程申请内存
	//	对象的时候必须去PageCache去取，带来效率问题
	
	else { //在对应位置处链表为空的话 从中心缓存获取
